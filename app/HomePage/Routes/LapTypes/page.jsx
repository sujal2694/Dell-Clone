...
const LapTypes = ({ }) => {
    const { laptops } = useContext(StoreContext);
    return (
            ...
            {laptops.map((item, index) => {
                return (
                    <div key={index} className=" group min-h-[330px] w-[400px] p-[25px] mb-[18px] flex items-center justify-center flex-col bg-blue-50 rounded-[10px]">
                        <Image className="max-w-[250px] max-h-[200px] h-[auto] border-0 mb-3 hover:scale-[1.1] duration-300" src={item.image} alt={item.name}></Image>
